...
import { TokenUsage, taskEventSchema, FebCodeEventName, EvalEventName } from "@evals/types"
...
export const useRunStatus = (run: Run) => {
	...
	const onMessage = useCallback((messageEvent: MessageEvent) => {
		...
		switch (eventName) {
			case FebCodeEventName.TaskStarted:
				startTimes.current.set(taskId, Date.now())
				break
			case FebCodeEventName.TaskTokenUsageUpdated: {
