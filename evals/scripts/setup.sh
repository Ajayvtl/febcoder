...
build_extension() {
  echo "🔨 Building the Feb Code extension..."
  cd ..
  mkdir -p bin
...
echo "🗄️ Syncing Feb Code evals database..."
...
if [[ ! -s "../bin/roo-code-latest.vsix" ]]; then
  ...
  read -p "💻 Do you want to build a new version of the Feb Code extension? (y/N): " build_extension
